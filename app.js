...
app.delete('/delete-printer/:ip', (req, res) => {
    ...
    try {
        // JSON dosyasını sil
        const jsonFile = path.join(__dirname, 'json_outputs', `${ip.replace(/\./g, '_')}.json`);
...
async function fetchAndSavePrinterData(ip) {
    ...
    try {
        ...
        if (error.code === 'ECONNABORTED') {
            throw new Error(`Yazıcıya erişim zaman aşımına uğradı (${ip}). Yazıcının açık olduğundan ve ağa bağlı olduğundan emin olun.`);
        } else if (error.code === 'ENOTFOUND') {
            throw new Error(`Yazıcı bulunamadı (${ip}). IP adresinin doğru olduğundan emin olun.`);
        } else if (error.code === 'ECONNREFUSED') {
            throw new Error(`Yazıcı bağlantısı reddedildi (${ip}). Yazıcının web arayüzünün açık olduğundan emin olun.`);
        } else {
...
function generateCSV() {
    ...
    let output = 'IP,Toner Adı,Yüzde Doluluk,Serial Number\n';
    let printerCount = 0;
    let tonerCount = 0;
            ...
            if (supplies) {
                for (const key in supplies) {
                    ...
                    if (color && percent !== undefined) {
                        output += `${ip},${color},${percent},${item?.partNumber?.trim() || 'Bilinmiyor'},${item?.pagesRemaining || 'Bilinmiyor'},${item?.maxCapacityPages || 'Bilinmiyor'}\n`;
                        tonerCount++;
                        hasPrinterData = true;
                    }
                    ...
                    if (key.includes('Photoconductor')) {
                        const photoconductorPercent = item?.percentFull !== undefined ? item.percentFull : 
                                                   (item?.curlevel !== undefined ? item?.curlevel : null);
                        
                        if (photoconductorPercent !== null) {
                            output += `${ip},${key},${photoconductorPercent},${item?.serialNumber?.trim() || 'Bilinmiyor'},${item?.pagesRemaining || 'Bilinmiyor'},${item?.maxCapacityPages || 'Bilinmiyor'}\n`;
                            tonerCount++;
                            hasPrinterData = true;
                        }
                    }
                    ...
                    if (key.includes('Kit')) {
                        const kitPercent = item?.percentFull !== undefined ? item.percentFull : 
                                         (item?.curlevel !== undefined ? item?.curlevel : null);
                        
                        if (kitPercent !== null) {
                            output += `${ip},${key},${kitPercent},${item?.serialNumber?.trim() || 'Bilinmiyor'},${item?.pagesRemaining || 'Bilinmiyor'},${item?.maxCapacityPages || 'Bilinmiyor'}\n`;
                            tonerCount++;
                            hasPrinterData = true;
                        }
                    }
...
async function updateAllPrinters() {
    try {
        const ipList = fs.readFileSync('ips.txt', 'utf-8').split('\n').filter(Boolean);
    ...
    try {
        ...
        if (supplies) {
            for (const key in supplies) {
                if (key.includes('Photoconductor')) {
                    const item = supplies[key];
                    const percent = item?.percentFull !== undefined ? item.percentFull : 
                                   (item?.curlevel !== undefined ? item?.curlevel : null);
